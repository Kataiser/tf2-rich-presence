...
def main():
    ...
    providers_page = get_with_retries('https://teamwork.tf/community/providers')
    ...
    for outer_div in providers_page_soup.find_all('div'):  # trying to find a link to each provider page
        if outer_div.get('class') == ['provider-tile-container']:
            ...
            provider_page = get_with_retries(provider_page_url)
...
def get_with_retries(url):
    ...
    while True:
        try:
            if is_providers_page:
                timeout_ = 10
            else:
                timeout_ = 2

            response = requests.get(url, timeout=timeout_).text
