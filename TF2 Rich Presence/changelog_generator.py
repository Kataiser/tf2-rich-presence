...
def main(silent=False):
    ...
    generated_html_soup = BeautifulSoup(''.join(extended_htmls), 'lxml')

    for a in generated_html_soup.find_all('a'):
        if not a.text:
            a.decompose()

    generated_html_pretty = prettify_custom(generated_html_soup).replace('<html>\n    <body>', '').replace('</body>\n</html>', '').replace('2019-11-28', '2019-12-09')
